...
public class BattleCanoes {
   
   /* POINTS: 1
      This method will be called when your other methods detect a problem.
      It shuts down the whole application after displaying an error message (parameter)
   ...
   */
   public static String renderBoard(int[] board){
      String str = "["; //Used to show string version of board  
      for (int i = 0; i < board.length; i++) { 
         str += renderCell(board[i]); 
      }//End of For loop 
      str += "]";
      return str;
   }// end method
   ...
   /* POINTS: 2
      ...
      Please note that the numbers are not consecutive. The values 4 and 5 are reserved
   ...
   */
   public static String renderCell(int value){
      String symbol; //Used to return string version of the int in value
      switch (value) {
         case 0: symbol = "~"; 
            break; 
         case 1: symbol = "\\"; 
   ...
   /* POINTS: 2
      ...
      another tabulation. After this will come a String representing player 1's board
      (2nd parameter). 
      ...
      Rubric
         ...
         1pt   Proper concatenation of all necessary data into the String being displayed
   */
   public static void displayBoards(String msg, int[] board1, int[] board2){
      System.out.print(msg + "\t" + "P1\t" + renderBoard(board1) + "\tP2" + renderBoard(board2)); 
   }// end method
   ...
   */
   public static void placeRandomCanoe(int[] board, int canoeSize){
      int start, end; //Determine place of canoe 
      if (canoeSize < 3 || canoeSize > board.length) 
         fail("Specified canoe sixe is not acceptable");  
      start = (int)Math.random() * (board.length - canoeSize);
      end = start + canoeSize;
      placeCanoe(board, start, end);  
   }// end of Place Random Canoe method
   ...
   */
   ...
   /* POINTS: 5
      ...
      6 (canoe's part has been shot, represented by "X") or 7 (previous shot in the water,
      represented by ".")
      ...
      of the array but did not find the Nth suitable location), then you have a bug.
   ...
   */
   public static int target(int[] board){
      int validSpots = 0; //Find number of valid spots
      int target; //Index of the target 
      int [] validLocations; //Array used to store the indexs of valid locations  
      for (int i = 0; i < board.length; i++) { 
         if(board[i] != 6 && board[i] != 7)  
            validSpots++; 
      }//End of For Loop
      if (validSpots == 0) 
         fail("Unable to take one more shot at this board:" + renderBoard(board)); 
      target = (int)Math.random() * validSpots;
      validLocations = new int [validSpots];
      ...
      if(target > validLocations.length - 1)
         fail("Unable to find random spot #" + target + " in this board" + renderBoard(board)); 
      return validLocations[target];
   }// end method
   ...
   /* POINTS: 4
      ...
      If the index is out of ranges, call the fail method with the following message;
                        "Specified position ("
      ...
      If none of these scenarios occur, you will then replace the value at the specified index,
   ...
   */
   public static void shoot(int[] board, int position) {
      if (position > board.length - 1) 
         fail("Specified Position(" + position + ") is outside this board: " + renderBoard(board));  
      if (board[position] == 6 || board[position] == 7)  
         fail("Specified Position(" + position + ") has already been shot in this board: " + renderBoard(board)); 
   ...
   */
   public static boolean sunken(int[] board){
      if (board.length == 0) 
         return true;
      for (int i = 0; i < board.length; i++ ) { 
         if (board[i] != 6 && board[i] != 7 && board[i] != 0)  
            return false; 
      }//End of For loop 
      return true;
   }// end method sunken   
