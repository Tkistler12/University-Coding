//This is code that the professor provided to assist with the exam
//Has a coouple methods and then test to run to test your code

import java.util.Arrays;

public class IE9Helper{
   ...
   public static void testTranslate(){
      ...
      if(numberOfTests != expectedResults.length)
         ...
         for(int i=0 ; i < numberOfTests ; i++)
            displayTestResult(i, expectedResults[i], StringToInteger.translate(inputs[i]), inputs[i]);
   ...
   */
   public static void testTranslateWordToValue(){
      ...
      for(int i=0 ; i < numbers.length ; i++,n++)
         displayTestResult(i, i, StringToInteger.translateWordToValue(numbers[i]), numbers[i]);
         
      String[] numbers2 = {  "twenty", "thirty", "fourty", "fifty", "sixty", "seventy", "eighty", "ninety", "hundred"};
      
      for(int i=0 ; i < numbers2.length ; i++,n++)
         displayTestResult(i+19, (i+2)*10, StringToInteger.translateWordToValue(numbers2[i]), numbers2[i]);
         
      displayTestResult(n, 1000, StringToInteger.translateWordToValue("thousand"), "thousand");
   ...
   */
   public static void testCleanUp(){
      ...
      String[] observed = StringToInteger.cleanUp(inputs);
   ...
   public static void testTranslateAllValues(){
      ...
      for(int i=0 ; i < inputs.length ; i++)
         displayTestResult(i, expected[i], StringToInteger.translateAllValues(inputs[i]), Arrays.toString(inputs[i]));
